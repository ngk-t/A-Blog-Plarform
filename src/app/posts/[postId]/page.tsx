...
export default async function Post({ params }: { params: { postId: string } }) {
  ...
  return (
    <main className=" flex min-h-screen flex-col items-center justify-center text-neutral-700">
      ...
      <div className="h-96 w-full rounded border-0 border-neutral-200 bg-orange-300 shadow-2xl">
        <img
      ...
      {/* </div> */}
      <div className="my-10 flex max-w-prose -translate-y-40 flex-col gap-4 rounded-xl bg-neutral-200/20 p-8 pb-12 shadow-xl backdrop-blur-3xl">
        <div className="space-y-4 whitespace-pre-line break-words">
          ...
          <p className="text-md flex items-center">
            <img src={`${data.createdBy.image}`}  alt="Profile Picture" className="w-10 h-10 rounded-full"/>
            <span className="font-bold ml-2">{`${data?.createdBy.name}`}</span> 
          </p>
          <p className="text-xs mb-10">{formattedDate}</p>

          <div
            className="space-y-4 text-justify indent-4 text-sm leading-relaxed"
            dangerouslySetInnerHTML={{ __html: data.content ?? "" }}
          />
          <p className="text-md text-right pt-4">
